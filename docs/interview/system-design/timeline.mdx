...
:::tip 推模式（Push）
**核心**：用户发微博，推送到所有粉丝的「收件箱」。

**优点**：读取快，用户请求直接返回。

**缺点**：写入压力大（大 V 发一条，推送 1000 万用户）。

**适用**：粉丝数相对均衡的产品。
:::

:::tip 拉模式（Pull）
**核心**：用户请求，聚合关注列表，拉取所有微博，排序返回。
...
```
用户请求 → API Gateway → Feed Service → 收件箱 Redis / 微博服务 / 用户关系服务
```
...
推拉模式各有优劣：推模式读快写慢，拉模式写快读慢。微博选择混合模式——普通用户推，大 V 拉——在读写压力之间找到平衡。
...
> 你有没有想过：微博的「智能排序」是如何工作的？点赞、评论、转发、亲密度——这些因素如何影响你的信息流排序？这个问题的答案，值得你深入了解推荐系统的原理。

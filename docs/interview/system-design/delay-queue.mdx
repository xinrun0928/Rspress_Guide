...
| 方案 | 适用场景 | 优点 | 缺点 |
|---|---|---|---|
| 定时轮询 | 不推荐 | 实现简单 | 数据库压力大，延迟不精确 |
| JDK ScheduledExecutorService | 不推荐 | 单机可用 | 集群环境有问题，任务分散 |
| 时间轮算法 | 生产环境推荐 | 高效、精确、支持批量处理 | 实现复杂，需要持久化支持 |
| Redis ZSet | 简单场景推荐 | 实现简单，性能高 | 不支持消息持久化，重启会丢失 |
| RabbitMQ/RocketMQ 延迟插件 | 生产环境推荐 | 消息队列原生支持 | 延迟时间有限制（RabbitMQ 插件最大 2^32 毫秒） |
...
> 时间轮算法是 Kafka、Netty、Redis 等高性能框架的核心组件。理解它，不仅能应对面试，更能理解这些框架为什么这么快。

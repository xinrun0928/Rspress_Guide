...
1. **partition 的时间复杂度一定是 O(n) 吗？**
   - 不是。最坏情况是 O(n²)（数组有序且选最右元素为基准）
   - 优化方案：随机选择基准，三数取中等

2. **堆排序的空间复杂度为什么是 O(1)？**
   - 因为堆排序是原地排序，只需要一个临时变量用于交换
   - 对比归并排序需要 O(n) 额外空间

3. **三路划分的适用场景是什么？**
   - 当数组中元素种类有限且已知时（如颜色分类只有 0、1、2）
   - 可以一次遍历将数组分成三部分，避免重复处理
...
**留一个思考题**：快排和归并都采用了分治思想，但一个递归两边，一个只递归一边。这个区别在实际应用中会带来什么影响？

如果你能说清楚「为什么快速选择可以做到平均 O(n)」，面试官对你的印象分就不会低。

...
```java title="docs/interview/algorithm/TrieNode.java"
class TrieNode {
    // 子节点，最多 26 个字母（如果是英文）
...
```java title="docs/interview/algorithm/TrieInsert.java"
...
```java title="docs/interview/algorithm/TrieSearch.java"
...
```java title="docs/interview/algorithm/TrieCountPrefix.java"
...
```java title="docs/interview/algorithm/Trie.java"
class Trie {
    private TrieNode root;
...
```java title="docs/interview/algorithm/TrieSearchWithWildcard.java"
...
```java title="docs/interview/algorithm/FindWords.java"
...
```java title="docs/interview/algorithm/FindMaximumXOR.java"
...
字典树的核心是**共享前缀，节省空间**。但你有没有想过——当字符串数量达到百万级时，字典树的内存占用会成为什么量级的问题？这个问题，值得在面试中和面试官深入探讨。

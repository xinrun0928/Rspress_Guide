...
```java title="SingleNumberII.java"
public int singleNumber(int[] nums) {
    int result = 0;
...
```java title="HammingWeight2.java"
public int hammingWeight(int n) {
    int count = 0;
    while (n != 0) {
        n &= (n - 1); // 消除最低位的 1
...
```java title="BitManipulation.java"
// 获取第 i 位的值（0 或 1）
...
```java title="CommonBitTricks.java"
// 判断奇偶：最后一位决定
boolean isOdd(int n) {
    return (n & 1) == 1;
}

...
想象你在走一个迷宫，每条路都试试，碰壁了就退回来换一条。这种「搜索 + 撤销」的过程，就是回溯。本质上，回溯是一种有剪枝的深度优先搜索（DFS），它解决的问题都有一个共同特征：**在多个选择中做决策，最终找出所有满足条件的组合**。
...
记住四类经典题型的模板：全排列用 used 数组标记已访问；组合从 start 开始递增；子集每个节点都记录结果；N 皇后用约束检查 + 按行递归。

到这里，回溯的套路你应该已经清楚了。但你有没有想过：同样是「搜索 + 撤销」，为什么有的问题用回溯，有的用 BFS？它们的本质区别是什么？这个问题，值得在 DFS 和 BFS 的章节里继续思考。
